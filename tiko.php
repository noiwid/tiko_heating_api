...
// v1    	2023-03-04
// v1.4  	2023-03-08
// v1.4.1	2023-04-10 - new sensor added with consumption difference in % (today vs last month same day)
// v1.5  	2023-06-12 - command_line sensors & switchs moved to separate section (to fit 2023.8 upcoming requirements) + minor bugfixes
// v1.5.1	2023-06-21 - fix warnings when no history data is provided + extend scan_interval to 60 seconds
// v1.5.2	2023-06-28 - code optimization
// v1.5.3	2023-06-29 - bug fix
// v1.5.4	2023-06-29 - unique_id added to climate entity to allow managing them from Lovelace UI + disable SSL certif validation
// v1.6.0	2023-11-26 - bug fix related to consommation comparaison sensors 
// v1.6.1	2023-11-30 - PHP Warning due to undefined vars fixed
// v1.7		2024-01-24 - Fix bug caused by API changes that disrupted temperature retrieval
// v1.8		2024-05-28 - bug fix to allow login with Tiko improved system (crsf cookie)
//====================================================================================================================================================
...
if(($hash and $_REQUEST["hash"]==$hash) or $_REQUEST["install"]){
	function f_tiko($json, $tokens=false, $account_id=false/*, $csrf_token=false, $member_space=false */){
		if(!$account_id) {
    	...
    	// Build headers if logued 
		if($tokens["token"])
			$headers[] = 'Authorization: Token '.$tokens["token"];
		if($tokens["csrf_token"] && $tokens["member_space"])
			$headers[] = 'Cookie: csrftoken='.$tokens["csrf_token"].'; USER_SESSION_member_space='.$tokens["member_space"];
		...
		//curl_setopt($chObj, CURLOPT_REFERER, 'https://particuliers-tiko.fr/dashboard/XXXXXXX'); // Set the Referer here
    ...
    /////////////
  	...
  	$login_response = f_tiko($json);
  	$login = $login_response['body'];

  	if($login_response['body']["errors"]){
  		die(json_encode(array("error"=>$login_response['body']["errors"][0]["message"])));
  	}

   	// Extract cookies from headers
  	preg_match_all('/^Set-Cookie:\s*([^;]*)/mi', $login_response['header'], $matches);
  	$cookies = array();
  	...
  	$tokens = array(
  		...
  		"member_space"=>$cookies['USER_SESSION_member_space']
  	);
   ...
   * GET GLOBAL ENERGY CONSUMPTION
	*******************************/
	if(!$_REQUEST["room_id"] and !isset($_REQUEST["mode"]) and $_REQUEST["consumption"]){
		$datas = f_tiko(false, $tokens, $account_id);
   ...
   **********************/
      ...
      if($_REQUEST["room_id"] and $_REQUEST["temperature"]>0){
      	$json = '{ 
      		...
      		$temperature = f_tiko($json, $tokens);  
      ...
      if(isset($_REQUEST["mode"])){
      	...
      	$json = '{ 
      		...
      		$feedback_mode = f_tiko($json, $tokens);  
